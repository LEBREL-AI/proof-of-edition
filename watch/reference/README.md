...
```
python -m watch.reference.prepare --prompts runs-fingerprint/<run>/prompts.json \
    --out runs-reference/prompts-<week>.json --check runs-fingerprint/<run>/summary.json
modal run watch/reference/app.py --action measure --prompts runs-reference/prompts-<week>.json \
    --out runs-reference/<id>.json
python -m watch.reference.analyze --result runs-reference/<id>.json --prompts runs-reference/prompts-<week>.json \
