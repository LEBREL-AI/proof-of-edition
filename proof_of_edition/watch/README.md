...
The audit in `proof_of_edition/audit/` lets a client re-execute its own receipts. The watch does the
...
python3 -m proof_of_edition.watch.run --targets proof_of_edition/watch/targets.json --out runs            # one run (~100 calls per target)
python3 -m proof_of_edition.watch.board --runs runs --out board.json --html board.html   # latest run vs previous
python3 -m proof_of_edition.watch.run --targets proof_of_edition/watch/targets.json --out runs --dry-run  # list probes and which keys are missing
```

```bash
python3 -m proof_of_edition.watch.calibrate --targets proof_of_edition/watch/targets.json --only deepseek/api   # the same-deployment noise floor
...
uploads the board to `proof_of_edition/watch/board.json` in the registry bucket (and keeps every run under
`proof_of_edition/watch/history/`); `models.lebrel.ai/watch` renders it and `/v1/watch` serves it raw.
