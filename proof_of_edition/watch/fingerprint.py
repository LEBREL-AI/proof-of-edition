...
Usage:
  python3 -m proof_of_edition.watch.fingerprint --targets targets.json --out runs-fingerprint [--only a,b] [--secret-pool pool.json]
"""
...
def route_receipt_checker(target: Target, *, opener=None, now: float | None = None) -> Callable[[bytes, str, str], str] | None:
    ...
    if not target.receipt_public_key or not target.route_model_id:
        return None
    from proof_of_edition.receipts.schema import Signed, check_route_manifest, check_route_receipt  # local import: optional dependency path
