...
key REGISTRY_PUBLISHER_SEED_HEX) only for signing the log checkpoint.
    ...
    uvx modal run proof_of_edition/registry/modal_publish.py --record proof_of_edition/registry/published/<edition>.record.json
"""
...
ROOT = HERE.parents[1]  # the repository root; the package is mounted whole so its imports resolve in the container
...
image = (
    ...
    .add_local_dir(str(ROOT / "proof_of_edition"), "/root/proof/proof_of_edition", copy=True)
)
...
def publish_edition(record_json: str, tag: str = "main", skip_verify: bool = False) -> dict:
    ...
    from proof_of_edition.registry.publish import S3Store, publish
